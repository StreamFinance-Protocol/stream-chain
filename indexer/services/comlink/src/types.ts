...
export interface PerpetualPositionWithFunding
  extends PerpetualPositionFromDatabase {
...
export type PerpetualPositionsMap = {
  [market: string]: PerpetualPositionResponseObject;
};
...
export interface OrderResponseObject
  extends Omit<
    OrderFromDatabase,
    'timeInForce' | 'status' | 'updatedAt' | 'updatedAtHeight'
  > {
...
export interface CandleResponseObject
  extends Omit<CandleFromDatabase, CandleColumns.id> {}
...
export interface LimitAndCreatedBeforeAndAfterRequest
  extends LimitAndCreatedBeforeRequest {
...
export interface ParentSubaccountAssetPositionRequest
...
export interface HistoricalFundingRequest
  extends LimitAndEffectiveBeforeRequest {
  ticker: string;
}
