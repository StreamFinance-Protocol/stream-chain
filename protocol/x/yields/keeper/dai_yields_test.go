...
func TestProcessNewSDaiConversionRateUpdate(t *testing.T) {
	testCases := []struct {
		...
		initialAssetYieldsIndex     string
		initialPerpYieldsIndexes    []string
		...
		expectedPerpYieldsIndexes      []string
	...
	}{
		{
			...
			initialAssetYieldsIndex:     "1/1",
			initialPerpYieldsIndexes:    []string{"0/1", "0/1"},
			...
			expectedAssetYieldsIndex:       "2/1",
			expectedPerpYieldsIndexes:      []string{"5/1", "3/1"},
			expEvent:                       true,
			expErr:                         false,
			customSetup:                    func(ctx sdk.Context, k *keeper.Keeper) {},
		},
		{
			name: "Valid: Large amounts and large new yields",
			...
			expectedAssetYieldsIndex:       "76207893903368/4938271605",
			expectedPerpYieldsIndexes:      []string{"76202955631763/987654321", "76202955631763/1646090535"},
			expEvent:                       true,
			expErr:                         false,
			customSetup:                    func(ctx sdk.Context, k *keeper.Keeper) {},
		},
		{
			...
			expectedAssetYieldsIndex:       "1/1",
			expectedPerpYieldsIndexes:      []string{"0/1", "0/1"},
			expEvent:                       false,
			expErr:                         false,
			customSetup:                    func(ctx sdk.Context, k *keeper.Keeper) {},
		},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			ctx, _, pricesKeeper, perpetualsKeeper, _, bankKeeper, assetsKeeper, yieldsKeeper, _, _ := testkeeper.SubaccountsKeepers(
				t,
				true,
			)
			...
			k.SetAssetYieldsIndex(ctx, keeper.ConvertStringToBigRatWithPanicOnErr(tc.initialAssetYieldsIndex))
			...
			for i, yieldsIndex := range tc.initialPerpYieldsIndexes {
				perp := allPerps[i]
				perp.YieldsIndex = yieldsIndex
			...
			if tc.expErr {
				...
				require.True(t, found)
				require.Equal(t, tc.expectedAssetYieldsIndex, assetYieldsIndex.String())

				allPerps := perpetualsKeeper.GetAllPerpetuals(ctx)
				for i, perpYieldsIndex := range tc.expectedPerpYieldsIndexes {
					require.Equal(t, perpYieldsIndex, allPerps[i].YieldsIndex)
				}

				actualEvents := testkeeper.GetUpdateYieldsParamsFromIndexerBlock(ctx, k)
				if tc.expEvent {
					require.Equal(t, 1, len(actualEvents))
					expectedEvent := indexerevents.UpdateYieldsParamsEventV1{
						SdaiPrice:        tc.newSDaiConversionRate.String(),
						AssetYieldsIndex: tc.expectedAssetYieldsIndex,
					}
...
func TestUpdateMintStateOnSDaiConversionRateUpdate(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedAssetYieldsIndex:  "10/1",
			expectedPerpYieldsIndexes: []string{"45/1", "27/1"},
			expErr:                    false,
			customSetup:               func(ctx sdk.Context, k *keeper.Keeper) {},
		},
		{
			name:                      "Basic: Small new yields",
			...
			expectedAssetYieldsIndex:  "10001/10000",
			expectedPerpYieldsIndexes: []string{"1/2000", "3/10000"},
			expErr:                    false,
			customSetup:               func(ctx sdk.Context, k *keeper.Keeper) {},
		},
		{
			name:                      "Large amounts: Large new yields",
			initialSDaiSupply:         keeper.ConvertStringToBigIntWithPanicOnErr("123456789123456789123456789"),
			initialTDaiSupply:         keeper.ConvertStringToBigIntWithPanicOnErr("9876543210"),
			...
			expectedAssetYieldsIndex:  "15241578780673/9876543210",
			expectedPerpYieldsIndexes: []string{"15231702237463/1975308642", "15231702237463/3292181070"},
			expErr:                    false,
			customSetup:               func(ctx sdk.Context, k *keeper.Keeper) {},
		},
		{
			name:                      "Large amounts: Small new yields",
			...
			expectedAssetYieldsIndex:  "997630627509664/997630627509663",
			expectedPerpYieldsIndexes: []string{"5/997630627509663", "1/332543542503221"},
			expErr:                    false,
			customSetup:               func(ctx sdk.Context, k *keeper.Keeper) {},
		},
		{
			name:                      "Initial AssetYieldsIndex non-zero: Large new yields",
			initialSDaiSupply:         keeper.ConvertStringToBigIntWithPanicOnErr("123456789123456789123456789"),
			initialTDaiSupply:         keeper.ConvertStringToBigIntWithPanicOnErr("9876543210"),
			sDaiConversionRate:        keeper.ConvertStringToBigIntWithPanicOnErr("123456789123456789123456789"),
			initialAssetYieldsIndex:   "876/543",
			initialPerpYieldsIndexes:  []string{"123456/123457", "98765/198765"},
			expectedTDAISupply:        keeper.ConvertStringToBigIntWithPanicOnErr("15241578780673"),
			expectedAssetYieldsIndex:  "2225270501978258/893827160505",
			expectedPerpYieldsIndexes: []string{"1880704126834176343/243865679015394", "201856963166180783/43624691358570"},
			expErr:                    false,
			customSetup:               func(ctx sdk.Context, k *keeper.Keeper) {},
		},
		{
			name:                      "Initial AssetYieldsIndex non-zero: Small new yields",
			initialSDaiSupply:         keeper.ConvertStringToBigIntWithPanicOnErr("987654321234567898765432123"),
			initialTDaiSupply:         keeper.ConvertStringToBigIntWithPanicOnErr("997630627509663"),
			sDaiConversionRate:        keeper.ConvertStringToBigIntWithPanicOnErr("1010101010101010101010101010"),
			initialAssetYieldsIndex:   "345/678",
			initialPerpYieldsIndexes:  []string{"9876/5432", "123456789/12345678"},
			expectedTDAISupply:        keeper.ConvertStringToBigIntWithPanicOnErr("997630627509664"),
			expectedAssetYieldsIndex:  "57363761081805680/112732260908591919",
			expectedPerpYieldsIndexes: []string{"351878574188766391/193540341736874622", "4561639773348077684783/456163944080453380982"},
			expErr:                    false,
			customSetup:               func(ctx sdk.Context, k *keeper.Keeper) {},
		},
		{
			...
			expectedAssetYieldsIndex:  "207602199060021/407983452065690",
			expectedPerpYieldsIndexes: []string{"4457128951994701/2451511185421270", "123815946305724809777/12381593727953401150"},
			expErr:                    false,
			customSetup:               func(ctx sdk.Context, k *keeper.Keeper) {},
		},
		{
		...
		{
			name:                      "Zero tDai Minted",
			initialSDaiSupply:         keeper.ConvertStringToBigIntWithPanicOnErr("987654321234567898765432123"),
			initialTDaiSupply:         keeper.ConvertStringToBigIntWithPanicOnErr("1083141908139240"),
			sDaiConversionRate:        keeper.ConvertStringToBigIntWithPanicOnErr("1096681181716810314385961731"),
			initialAssetYieldsIndex:   "115/226",
			initialPerpYieldsIndexes:  []string{"9876/5432", "123456789/12345678"},
			expectedTDAISupply:        keeper.ConvertStringToBigIntWithPanicOnErr("1083141908139240"),
			expectedAssetYieldsIndex:  "115/226",
			expectedPerpYieldsIndexes: []string{"9876/5432", "123456789/12345678"},
			expErr:                    false,
			customSetup:               func(ctx sdk.Context, k *keeper.Keeper) { expectEvent = false },
		},
		{
			name:                      "Failure: Lower tDai amount after yields",
			initialSDaiSupply:         keeper.ConvertStringToBigIntWithPanicOnErr("987654321234567898765432123"),
			...
			expectedAssetYieldsIndex:  "345/678",
			expectedPerpYieldsIndexes: []string{"9876/5432", "123456789/12345678"},
			expErr:                    true,
			customSetup:               func(ctx sdk.Context, k *keeper.Keeper) {},
		},
		{
			...
			initialPerpYieldsIndexes:  []string{"9876/5432", "123456789/12345678"},
			expectedTDAISupply:        keeper.ConvertStringToBigIntWithPanicOnErr("1083141908139240"),
			expectedAssetYieldsIndex:  "9026182567827/9026182567825",
			expectedPerpYieldsIndexes: []string{"4457128951994701/2451511185421270", "123815946305724809777/12381593727953401150"},
			expErr:                    false,
			customSetup: func(ctx sdk.Context, k *keeper.Keeper) {
...
func TestClaimInsuranceFundYields(t *testing.T) {
	ctx, _, pricesKeeper, perpetualsKeeper, _, bankKeeper, assetsKeeper, yieldsKeeper, _, _ := testkeeper.SubaccountsKeepers(t, true)
	...
	tradingDaiMinted := big.NewInt(10_000_000_000)                  // 10,000 tDAI (1% yields)

	// Call the function
	err = yieldsKeeper.ClaimInsuranceFundYields(ctx, tradingDaiSupplyBeforeNewEpoch, tradingDaiMinted)
	require.NoError(t, err)

	// Check results
	expectedCrossFundYields := big.NewInt(1_000_000_000)   // 1% of 100,000 tDAI
	expectedIsolatedFund1Yields := big.NewInt(500_000_000) // 1% of 50,000 tDAI
	expectedIsolatedFund2Yields := big.NewInt(250_000_000) // 1% of 25,000 tDAI
	...
	require.Equal(t, 0, crossFundBalanceAfter.Amount.BigInt().Cmp(new(big.Int).Add(crossFundBalance.Amount.BigInt(), expectedCrossFundYields)))
	require.Equal(t, 0, isolatedFund1BalanceAfter.Amount.BigInt().Cmp(new(big.Int).Add(isolatedFund1Balance.Amount.BigInt(), expectedIsolatedFund1Yields)))
	require.Equal(t, 0, isolatedFund2BalanceAfter.Amount.BigInt().Cmp(new(big.Int).Add(isolatedFund2Balance.Amount.BigInt(), expectedIsolatedFund2Yields)))
}

func TestSetNewYieldsIndex(t *testing.T) {
	testCases := []struct {
		name                     string
		totalTDaiPreMint         *big.Int
		totalTDaiMinted          *big.Int
		initialAssetYieldsIndex  *big.Rat
		expectedAssetYieldsIndex *big.Rat
		expectErr                bool
	...
	}{
		{
			...
			initialAssetYieldsIndex:  keeper.ConvertStringToBigRatWithPanicOnErr("1"),
			expectedAssetYieldsIndex: keeper.ConvertStringToBigRatWithPanicOnErr("1.5"),
			expectErr:                false,
			customSetup:              func(ctx sdk.Context, tApp *testapp.TestApp) {},
		},
		{
			...
			initialAssetYieldsIndex:  keeper.ConvertStringToBigRatWithPanicOnErr("1"),
			expectedAssetYieldsIndex: keeper.ConvertStringToBigRatWithPanicOnErr("500001"),
			expectErr:                false,
			customSetup:              func(ctx sdk.Context, tApp *testapp.TestApp) {},
		},
		{
			...
			initialAssetYieldsIndex:  keeper.ConvertStringToBigRatWithPanicOnErr("1"),
			expectedAssetYieldsIndex: keeper.ConvertStringToBigRatWithPanicOnErr("4115227/4115226"),
			expectErr:                false,
			customSetup:              func(ctx sdk.Context, tApp *testapp.TestApp) {},
		},
		{
			...
			initialAssetYieldsIndex:  keeper.ConvertStringToBigRatWithPanicOnErr("1.2345"),
			expectedAssetYieldsIndex: keeper.ConvertStringToBigRatWithPanicOnErr("1.2345"),
			expectErr:                false,
			customSetup:              func(ctx sdk.Context, tApp *testapp.TestApp) {},
		},
		{
		...
		},
		{
			...
			initialAssetYieldsIndex:  keeper.ConvertStringToBigRatWithPanicOnErr("1"),
			expectedAssetYieldsIndex: keeper.ConvertStringToBigRatWithPanicOnErr("1"),
			expectErr:                true,
		...
		{
			name:                     "Cannot find asset yields index, handled gracefully",
			totalTDaiPreMint:         keeper.ConvertStringToBigIntWithPanicOnErr("1"),
			totalTDaiMinted:          keeper.ConvertStringToBigIntWithPanicOnErr("2"),
			initialAssetYieldsIndex:  keeper.ConvertStringToBigRatWithPanicOnErr("1"),
			expectedAssetYieldsIndex: keeper.ConvertStringToBigRatWithPanicOnErr("3/1"),
			expectErr:                false,
			customSetup: func(ctx sdk.Context, tApp *testapp.TestApp) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ctx := tApp.InitChain()
			k := tApp.App.YieldsKeeper
			k.SetAssetYieldsIndex(ctx, tc.initialAssetYieldsIndex)
			tc.customSetup(ctx, tApp)

			err := k.SetNewYieldsIndex(ctx, tc.totalTDaiPreMint, tc.totalTDaiMinted)
			resultAssetYieldsIndex, found := k.GetAssetYieldsIndex(ctx)

			if tc.expectErr && tc.expectedAssetYieldsIndex == nil {
			...
			if tc.expectErr {
				...
				if tc.expectedAssetYieldsIndex != nil {
					require.Equal(t, 0, tc.initialAssetYieldsIndex.Cmp(resultAssetYieldsIndex), "Expected AssetYieldsIndex: %v. Got: %v.", tc.initialAssetYieldsIndex, resultAssetYieldsIndex)
				}
			} else {
				require.NoError(t, err)
				require.Equal(t, 0, tc.expectedAssetYieldsIndex.Cmp(resultAssetYieldsIndex), "Expected AssetYieldsIndex: %v. Got: %v.", tc.expectedAssetYieldsIndex, resultAssetYieldsIndex)
			}
		})
	}
}

func TestMintNewTDaiYields(t *testing.T) {
	testCases := []struct {
		name                     string
		...
		{
			name:                    "FAILS: tradingDaiAfterYields will be less than initial trading dai",
		...
		{
			name:                     "FAILS: Price results in lower post-yields tDAI Amount",
			initialSDAISupply:        sdk.NewCoins(sdk.NewCoin(types.SDaiDenom, sdkmath.NewInt(100))),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			tDAISupply, tradingDaiToMint, err := k.MintNewTDaiYields(ctx)
			if tc.expectError {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
