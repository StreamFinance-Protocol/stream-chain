...
func TestProcessNewSDaiConversionRateUpdate(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			ctx, _, pricesKeeper, perpetualsKeeper, _, bankKeeper, assetsKeeper, yieldKeeper, _, _ := testkeeper.SubaccountsKeepers(
				t,
				true,
			)
...
func TestUpdateMintStateOnSDaiConversionRateUpdate(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			sDaiConversionRate:       keeper.ConvertStringToBigIntWithPanicOnErr("1096681181716810314385961731"),
			initialAssetYieldIndex:   "115/226",
			initialPerpYieldIndexes:  []string{"9876/5432", "123456789/12345678"},
			expectedTDAISupply:       keeper.ConvertStringToBigIntWithPanicOnErr("1083141908139240"),
			expectedAssetYieldIndex:  "115/226",
...
func TestClaimInsuranceFundYields(t *testing.T) {
	ctx, _, pricesKeeper, perpetualsKeeper, _, bankKeeper, assetsKeeper, yieldKeeper, _, _ := testkeeper.SubaccountsKeepers(t, true)
	...
	// Call the function
	err = yieldKeeper.ClaimInsuranceFundYields(ctx, tradingDaiSupplyBeforeNewEpoch, tradingDaiMinted)
	require.NoError(t, err)
