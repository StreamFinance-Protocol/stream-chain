...
export async function up(knex: Knex): Promise<void> {
  return knex.schema.createTable('perpetual_markets', (table) => {
    table.bigInteger('id').primary();
    table.bigInteger('clobPairId').notNullable();
    table.string('ticker').notNullable();
    table.integer('marketId').notNullable();
    table
      .enum('status', ['ACTIVE', 'PAUSED', 'CANCEL_ONLY', 'POST_ONLY'])
      .notNullable();
    ...
    table.integer('collateralPoolId').notNullable();
    ...
    table.string('perpYieldsIndex').notNullable();
  });
}
