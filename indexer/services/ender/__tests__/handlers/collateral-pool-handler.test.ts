...
const { getCollateralPoolFromId } = collateralPoolRefresher;
  ...
  describe('collateralPoolHandlerV1', () => {
    describe('getParallelizationIds', () => {
      it('returns the correct parallelization ids', () => {
        ...
        const handler: CollateralPoolCreationHandler = new CollateralPoolCreationHandler(
          block,
          0,
          indexerTendermintEvent,
          0,
    ...
    it('creates new collateral pool', async () => {
      const transactionIndex: number = 0;
      const collateralPoolEvent: CollateralPoolUpsertEvent = defaultCreateCollateralPoolEvent;
      const kafkaMessage: KafkaMessage = createKafkaMessageFromCollateralPoolUpsertEvent({
        collateralPoolEvent:
      ...
      const newCollateralPools: CollateralPoolFromDatabase[] = await CollateralPoolsTable.findAll(
        {},
        [],
        {
      ...
      expectCollateralPoolMatchesEvent(newCollateralPools[0], collateralPoolEvent);
...
export function expectCollateralPoolMatchesEvent(
  collateralPoolFromDb: CollateralPoolFromDatabase,
  event: CollateralPoolUpsertEvent,
): void {
  expect(collateralPoolFromDb.id).toEqual(event.id);
  // Convert the Long type to a string for comparison
  expect(collateralPoolFromDb.maxCumulativeInsuranceFundDeltaPerBlock).toEqual(
    event.maxCumulativeInsuranceFundDeltaPerBlock.toString(),
  );
...
function validateCollateralPoolRefresher(
  ...
  const collateralPool: CollateralPoolFromDatabase = getCollateralPoolFromId(
    collateralPoolEvent.id,
  );
  ...
  expect(collateralPool.maxCumulativeInsuranceFundDeltaPerBlock).toEqual(
    collateralPoolEvent.maxCumulativeInsuranceFundDeltaPerBlock.toString(),
  );
