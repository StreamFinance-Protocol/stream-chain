...
const (
	...
	// Use `yields` instead of `yields` to prevent potential key space conflicts with the IBC module.
	ModuleName = "yields"
	...
	// TDaiPool: Address, where tDAI yields is held before it is claimed by subaccount.
	...
	// AssetYieldsIndexPrefix is the prefix for the key value store that tracks
	// the cumulative yields index across all yields epochs.
	AssetYieldsIndexPrefix = "AssetYieldsIndex:"
)
